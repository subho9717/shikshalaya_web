...
def COMPUTER_SALLARY(request):
    ...
    # conn = connect(user='shikshalayaweb', password='rana@8616', host = 'shikshalayaweb.mysql.pythonanywhere-services.com' ,database = 'shikshalayaweb$shikshalaya_web')
    # cursor = conn.cursor()
    ...
    ce = pd.read_sql_query(
        'select month,sum("Amount") as expenses_amount from shikshalaya_computer_app_computer_expenses group by month',
        conn)
    ...
    fnl['rupam_subhajit_share'] = (fnl['month_fees'] * 55) / 100 + fnl['registration_fees_share'] - fnl[
        'expenses_share']
    fnl = round(fnl, 1)
